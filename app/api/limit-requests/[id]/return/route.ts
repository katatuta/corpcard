...
// POST - 미사용분 반환 (요청자 본인만, FULFILLED 또는 PARTIAL 상태)
export async function POST(
  ...
  try {
    ...
    if (limitRequest.status !== "FULFILLED" && limitRequest.status !== "PARTIAL") {
      return NextResponse.json({ error: "충족 또는 부분 확정된 요청만 반환할 수 있습니다." }, { status: 400 });
    }

    // 확정 시각 이후 실제 사용한 금액 계산
    ...
    // 실사용분은 approvedTotal 초과 불가 (PARTIAL은 approvedTotal이 실제 가용 금액)
    const actualUsed = Math.min(
      usedAfter._sum.amount ?? 0,
      limitRequest.approvedTotal
    );
    const totalToReturn = limitRequest.approvedTotal - actualUsed;
